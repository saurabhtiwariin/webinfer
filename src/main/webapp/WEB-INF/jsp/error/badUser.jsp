...
<body>
	<div class="container">
		<h1 class="alert alert-danger">${param.message}</h1>
		...
		<c:if test="${param.expired}">
			...
			<script type="text/javascript">
				function resendToken() {
					$
							.get(
									"<c:url value="/userRegister/resendRegistrationToken"><c:param name="token" value="${param.token}"/></c:url>",
									function(data) {
										window.location.href = "<c:url value="/error/login.html"></c:url>"
							...
							.fail(
									function(data) {
										if (data.responseJSON.error
												.indexOf("MailError") > -1) {
											window.location.href = "<c:url value="/error/emailError.html"></c:url>";
										} else {
											window.location.href = "<c:url value="/error/login.html"></c:url>"
													+ "?message="
